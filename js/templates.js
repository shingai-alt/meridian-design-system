...
function tplSide(active){
  ...
  return `<button class="tsb-item" style="font-weight:600;width:100%;margin-bottom:6px">${avatar({fallback:'AC',alt:'',size:'xs'})} Acme Inc. <span style="margin-left:auto">${I.chevD}</span></button>
  ...
  <div style="margin-top:16px;border-top:1px solid var(--border-muted);padding-top:8px">${it('',`${avatar({fallback:'SN',alt:'',size:'xs'})} 新谷`,false)}</div>`}
const topSearch=(title)=>`${crumbsEl({items:['Acme',title]})}<div style="margin-left:auto"></div><div class="input" data-size="sm" style="width:200px"><span class="affix">${I.search}</span><input placeholder="検索"><span class="affix"><kbd style="border:1px solid var(--border);border-radius:var(--radius-2xs);padding:0 4px;font-size:9px">⌘K</kbd></span></div><button class="tb-btn" aria-label="通知">${I.bell}</button>${avatar({fallback:'SN',alt:'新谷 尚史',size:'sm'})}`;
...
<div class="rowflex">${btn({label:'⌘K',variant:'secondary',size:'sm'})}${btn({label:'フィルタ',variant:'secondary',size:'sm'})}<span class="avstack" role="img" aria-label="メンバー: 新谷、Yuki、ほか3名"><span aria-hidden="true">${avatar({fallback:'SN',alt:'',size:'xs'})}${avatar({fallback:'YK',alt:'',size:'xs'})}<span class="avcount">+3</span></span></span>${btn({label:'タスクを追加',size:'sm'})}</div></div>
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--sp-3);align-items:start">
${[['Todo','3',[taskCard()]],['In Progress','2',[`<div class="cardc" style="padding:12px"><div class="rowflex" style="gap:4px;margin-bottom:6px">${badge({label:'feature',tone:'primary'})}</div><div style="font-weight:550;font-size:var(--text-label);margin-bottom:8px">Playground に Density selector を追加</div><div class="rowflex" style="justify-content:space-between"><span style="font-family:var(--font-mono);font-size:10.5px;color:var(--fg-subtle)">MRD-141</span><span aria-label="担当: 新谷 尚史">${avatar({fallback:'SN',alt:'',size:'xs'})}</span></div></div>`]],['Done','8',[`<div class="cardc" style="padding:12px;opacity:.7"><div style="font-weight:550;font-size:var(--text-label);margin-bottom:8px;text-decoration:line-through">Seed picker の実装</div><div class="rowflex" style="justify-content:space-between"><span style="font-family:var(--font-mono);font-size:10.5px;color:var(--fg-subtle)">MRD-120</span>${badge({label:'Done',tone:'success'})}</div></div>`]]].map(([t,n,cards])=>`
...
${[['新谷','SN','sn@acme.co','Admin',['success','Active'],'2 分前'],['Yuki Kato','YK','yuki@acme.co','Editor',['success','Active'],'1 時間前'],['Mori Yusuke','MY','mori@acme.co','Viewer',['idle','Invited'],'—']].map(r=>`<tr><td><div class="rowflex" style="gap:10px;flex-wrap:nowrap">${avatar({fallback:r[1],alt:'',size:'sm'})}<div><div style="font-weight:550">${r[0]}</div><div style="font-size:var(--text-micro);color:var(--fg-subtle)">${r[2]}</div></div></div></td>
<td><div class="input" data-size="sm" style="width:120px"><select><option ${r[3]==='Admin'?'selected':''}>Admin</option><option ${r[3]==='Editor'?'selected':''}>Editor</option><option ${r[3]==='Viewer'?'selected':''}>Viewer</option></select></div></td>
<td>${statusEl({s:r[4][0],label:r[4][1]})}</td><td style="color:var(--fg-subtle)">${r[5]}</td><td><button class="tb-btn" aria-label="メニュー">${I.dots}</button></td></tr>`).join('')}
</tbody></table></div>
...
<div class="tpl-top" style="position:sticky;top:0"><button class="tb-btn" aria-label="メニュー">${I.menu}</button><b style="font-size:var(--text-label)">ダッシュボード</b><div style="margin-left:auto" class="rowflex"><button class="tb-btn">${I.search}</button>${avatar({fallback:'SN',alt:'新谷 尚史',size:'sm'})}</div></div>
