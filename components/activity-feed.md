...
ユーザー行動のフィード。アバター+行動+時刻の定型で表示する。

Machine-readable contract: `design/contracts/components/activity-feed.contract.json`

## Role

Data Display領域でActivity Feedの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- 同種イベントの連続は「SN が 3 件のファイルを更新」と集約する。
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.surface-muted.value` | `--surface-muted` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.border-muted.value` | `--border-muted` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
...
| `token.row-h.value` | `--row-h` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.cell-y.value` | `--cell-y` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.focus-ring.value` | `--focus-ring` | `semantic` | - | Activity Feedの公開visual contractで用途tokenとして共有する。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | Activity FeedのHTML showcaseで実際に参照する公開token。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | Activity FeedのHTML showcaseで実際に参照する公開token。 |
| `token.text-micro.value` | `--text-micro` | `semantic` | - | Activity FeedのHTML showcaseで実際に参照する公開token。 |
...
```tsx
<ActivityFeed items={events} groupBy="hour" />
```
