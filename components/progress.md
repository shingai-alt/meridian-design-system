...
進捗率が分かる処理の可視化。残り時間の目安があれば併記する。

Machine-readable contract: `design/contracts/components/progress.contract.json`

## Role

Feedback領域でProgressの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- 複数ステップの処理は Workflow Step と組み合わせる。
...
- radius: `--radius-full`
- motion: `--dur-normal`
- typography: `--font-mono`, `--text-small`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.surface-muted.value` | `--surface-muted` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.primary.value` | `--primary` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.success.value` | `--success` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.warning.value` | `--warning` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.danger.value` | `--danger` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.sp-2.value` | `--sp-2` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.radius-full.value` | `--radius-full` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.dur-normal.value` | `--dur-normal` | `semantic` | - | Progressの公開visual contractで用途tokenとして共有する。 |
| `token.font-mono.value` | `--font-mono` | `semantic` | - | ProgressのHTML showcaseで実際に参照する公開token。 |
| `token.text-small.value` | `--text-small` | `semantic` | - | ProgressのHTML showcaseで実際に参照する公開token。 |
...
```tsx
<Progress value={64} label="アップロード中" />
```
