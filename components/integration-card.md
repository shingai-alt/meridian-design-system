...
外部連携のカード。接続済み・要設定・未接続の状態を持つ。

Machine-readable contract: `design/contracts/components/integration-card.contract.json`

## Role

SaaS領域でIntegration Cardの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
| Variant | Use | Notes |
|---|---|---|
| `connected` | connectedという明示的な意味を持つ文脈。 | 意味を変えずに見た目だけを増やさない。 |
| `setup` | setupという明示的な意味を持つ文脈。 | 意味を変えずに見た目だけを増やさない。 |
| `none` | noneという明示的な意味を持つ文脈。 | 意味を変えずに見た目だけを増やさない。 |
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.card-pad.value` | `--card-pad` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | Integration Cardの公開visual contractで用途tokenとして共有する。 |
| `token.text-body.value` | `--text-body` | `semantic` | - | Integration CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-small.value` | `--text-small` | `semantic` | - | Integration CardのHTML showcaseで実際に参照する公開token。 |
