...
ダッシュボードの主要指標。値・変化量・スパークラインをまとめる。

Machine-readable contract: `design/contracts/components/kpi-card.contract.json`

## Role

SaaS領域でKPI Cardの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.card-pad.value` | `--card-pad` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | KPI Cardの公開visual contractで用途tokenとして共有する。 |
| `token.chart-1.value` | `--chart-1` | `semantic` | - | KPI CardのHTML showcaseで実際に参照する公開token。 |
