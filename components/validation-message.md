...
入力エラー・成功の通知。アイコン+テキストで色覚に依存しない。

Machine-readable contract: `design/contracts/components/validation-message.contract.json`

## Role

Forms領域でValidation Messageの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- 送信時にまとめて出す場合は、最初のエラーへフォーカスを移動する。
...
- semanticColor: `--border`, `--danger-fg`, `--danger-subtle`, `--fg`, `--fg-muted`, `--info-fg`, `--info-subtle`, `--success-fg`, `--success-subtle`, `--surface`, `--warning-fg`, `--warning-subtle`
- spacing: `--sp-3`
- radius: `--radius-md`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.info-subtle.value` | `--info-subtle` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.info-fg.value` | `--info-fg` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.success-subtle.value` | `--success-subtle` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.success-fg.value` | `--success-fg` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.warning-subtle.value` | `--warning-subtle` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.warning-fg.value` | `--warning-fg` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.danger-subtle.value` | `--danger-subtle` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.danger-fg.value` | `--danger-fg` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.sp-3.value` | `--sp-3` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | Validation Messageの公開visual contractで用途tokenとして共有する。 |
| `token.text-small.value` | `--text-small` | `semantic` | - | Validation MessageのHTML showcaseで実際に参照する公開token。 |
