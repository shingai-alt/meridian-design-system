...
定義済みの選択肢から 1 つを選ぶ。5〜15 個程度の選択肢に向く。

Machine-readable contract: `design/contracts/components/select.contract.json`

## Role

Forms領域でSelectの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- 「選択してください」を初期値にする場合は必須バリデーションを付ける。
...
- `Arrow Up / Down`: Optionを移動する。
- `Enter / Space`: Optionを選択する。
- `Escape`: 選択を変えずに閉じる。
...
- density: `--ctl-md`, `--ctl-sm`, `--ctl-lg`, `--text-label`
- spacing: `--sp-2`
- radius: `--radius-sm`
- motion: `--dur-fast`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.surface-muted.value` | `--surface-muted` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.border-strong.value` | `--border-strong` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.primary.value` | `--primary` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.primary-subtle.value` | `--primary-subtle` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.focus-ring.value` | `--focus-ring` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-md.value` | `--ctl-md` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.sp-2.value` | `--sp-2` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.radius-sm.value` | `--radius-sm` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.dur-fast.value` | `--dur-fast` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-sm.value` | `--ctl-sm` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-lg.value` | `--ctl-lg` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | Selectの公開visual contractで用途tokenとして共有する。 |
...
```tsx
<Select label="ロール" defaultValue="editor">
