...
関連情報を 1 つの面にまとめる基本コンテナ。

Machine-readable contract: `design/contracts/components/card.contract.json`

## Role

Data Display領域でCardの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- padding は card-padding トークンで density に連動させる。
- クリック可能なカードは hover でボーダーを強調し、影は控えめに。
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.card-pad.value` | `--card-pad` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | Cardの公開visual contractで用途tokenとして共有する。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-small.value` | `--text-small` | `semantic` | - | CardのHTML showcaseで実際に参照する公開token。 |
