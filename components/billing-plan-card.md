...
料金プランの比較カード。現在のプランを明示する。

Machine-readable contract: `design/contracts/components/billing-plan-card.contract.json`

## Role

SaaS領域でBilling Plan Cardの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- semanticColor: `--border`, `--fg`, `--fg-muted`, `--fg-subtle`, `--success`, `--surface`
- density: `--card-pad`
- spacing: `--sp-4`
- radius: `--radius-md`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.card-pad.value` | `--card-pad` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | Billing Plan Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | Billing Plan CardのHTML showcaseで実際に参照する公開token。 |
| `token.success.value` | `--success` | `semantic` | - | Billing Plan CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-small.value` | `--text-small` | `semantic` | - | Billing Plan CardのHTML showcaseで実際に参照する公開token。 |
