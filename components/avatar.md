...
ユーザー・チームを表す円形の識別子。イニシャルまたは画像。

Machine-readable contract: `design/contracts/components/avatar.contract.json`

## Role

Core領域でAvatarの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- density: `--ctl-xs`, `--ctl-sm`, `--ctl-md`, `--ctl-lg`, `--text-label`
- spacing: `--sp-2`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-xs.value` | `--ctl-xs` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-sm.value` | `--ctl-sm` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-md.value` | `--ctl-md` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-lg.value` | `--ctl-lg` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
| `token.sp-2.value` | `--sp-2` | `semantic` | - | Avatarの公開visual contractで用途tokenとして共有する。 |
...
```tsx
<Avatar name="新谷" size="md" />
<AvatarStack max={3} users={members} />
```
