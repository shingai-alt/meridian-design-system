...
AI の提案・分析結果を採用/却下できるカードとして提示する。

Machine-readable contract: `design/contracts/components/ai-response-card.contract.json`

## Role

AI & Developer領域でAI Response Cardの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- AI 生成であることをラベルで明示する。
- 「適用」「編集」「破棄」の 3 操作を標準にする。
- 確信度・根拠(参照ファイル等)を開示できるようにする。
...
- semanticColor: `--border`, `--fg`, `--fg-muted`, `--fg-subtle`, `--primary`, `--surface`
- density: `--card-pad`
- spacing: `--sp-4`
- radius: `--radius-md`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.card-pad.value` | `--card-pad` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | AI Response Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | AI Response CardのHTML showcaseで実際に参照する公開token。 |
| `token.primary.value` | `--primary` | `semantic` | - | AI Response CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | AI Response CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-micro.value` | `--text-micro` | `semantic` | - | AI Response CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-small.value` | `--text-small` | `semantic` | - | AI Response CardのHTML showcaseで実際に参照する公開token。 |
