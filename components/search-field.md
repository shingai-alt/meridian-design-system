...
検索専用の入力。ショートカットヒントとクリア操作を持つ。

Machine-readable contract: `design/contracts/components/search-field.contract.json`

## Role

Forms領域でSearch Fieldの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- `Escape`: 検索語をclearするか、空ならfocusを維持する。
...
- semanticColor: `--border`, `--danger`, `--fg`, `--fg-disabled`, `--fg-muted`, `--fg-subtle`, `--focus-ring`, `--input-bg`, `--input-border`, `--input-border-focus`, `--input-placeholder`, `--primary`, `--surface`
- density: `--ctl-md`, `--ctl-sm`, `--input-x`, `--text-label`
- spacing: `--sp-2`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
...
| `token.primary.value` | `--primary` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.input-placeholder.value` | `--input-placeholder` | `component` | `component-anatomy` | Input固有のplaceholder anatomyへ安定してbindingする。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.fg-disabled.value` | `--fg-disabled` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.danger.value` | `--danger` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.focus-ring.value` | `--focus-ring` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-md.value` | `--ctl-md` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.input-x.value` | `--input-x` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.radius-sm.value` | `--radius-sm` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.dur-fast.value` | `--dur-fast` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.ctl-sm.value` | `--ctl-sm` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.sp-2.value` | `--sp-2` | `semantic` | - | Search Fieldの公開visual contractで用途tokenとして共有する。 |
| `token.radius-2xs.value` | `--radius-2xs` | `semantic` | - | Search FieldのHTML showcaseで実際に参照する公開token。 |
...
```tsx
<SearchField placeholder="コンポーネントを検索" shortcut="⌘K" onSearch={...} />
```
