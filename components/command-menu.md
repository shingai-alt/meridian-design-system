...
⌘K で開くグローバルなコマンドパレット。ナビゲーションと操作を統合する。

Machine-readable contract: `design/contracts/components/command-menu.contract.json`

## Role

Navigation領域でCommand Menuの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- 最近使ったコマンドを先頭グループに出す。
- 結果はカテゴリでグループ化し、ショートカットを右端に表示する。
- 0 件時は「一致するコマンドがありません」と入力例を示す。
...
- `Arrow Up / Down`: Commandを移動する。
- `Enter`: Active commandを実行する。
- `Escape`: 閉じて呼び出し元へfocusを戻す。
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.surface-overlay.value` | `--surface-overlay` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.overlay.value` | `--overlay` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.shadow-overlay.value` | `--shadow-overlay` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.focus-ring.value` | `--focus-ring` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.dur-normal.value` | `--dur-normal` | `semantic` | - | Command Menuの公開visual contractで用途tokenとして共有する。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | Command MenuのHTML showcaseで実際に参照する公開token。 |
| `token.radius-2xs.value` | `--radius-2xs` | `semantic` | - | Command MenuのHTML showcaseで実際に参照する公開token。 |
