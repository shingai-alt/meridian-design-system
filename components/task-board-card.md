...
カンバンボードのタスクカード。ドラッグ対応を想定した密度。

Machine-readable contract: `design/contracts/components/task-board-card.contract.json`

## Role

SaaS領域でTask Board Cardの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- semanticColor: `--border`, `--fg`, `--fg-muted`, `--fg-subtle`, `--surface`
- density: `--card-pad`
- spacing: `--sp-4`
- radius: `--radius-md`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.card-pad.value` | `--card-pad` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.sp-4.value` | `--sp-4` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.radius-md.value` | `--radius-md` | `semantic` | - | Task Board Cardの公開visual contractで用途tokenとして共有する。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | Task Board CardのHTML showcaseで実際に参照する公開token。 |
| `token.font-mono.value` | `--font-mono` | `semantic` | - | Task Board CardのHTML showcaseで実際に参照する公開token。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | Task Board CardのHTML showcaseで実際に参照する公開token。 |
...
```tsx
<TaskCard task={task} draggable onOpen={open} />
```
...
- `DRAG_ALTERNATIVE_REQUIRED`に反する実装。
