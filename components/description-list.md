...
Machine-readable contract: `design/contracts/components/description-list.contract.json`

## Role

Data Display領域でDescription Listの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- semanticColor: `--border`, `--border-muted`, `--fg`, `--fg-muted`, `--focus-ring`, `--surface`, `--surface-muted`, `--table-row-hover`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.surface-muted.value` | `--surface-muted` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.border-muted.value` | `--border-muted` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
...
| `token.row-h.value` | `--row-h` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.cell-y.value` | `--cell-y` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.focus-ring.value` | `--focus-ring` | `semantic` | - | Description Listの公開visual contractで用途tokenとして共有する。 |
| `token.text-label.value` | `--text-label` | `semantic` | - | Description ListのHTML showcaseで実際に参照する公開token。 |
