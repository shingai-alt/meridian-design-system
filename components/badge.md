...
状態やカテゴリを示す小さなラベル。読み取り専用で、操作は持たない。

Machine-readable contract: `design/contracts/components/badge.contract.json`

## Role

Core領域でBadgeの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
| Variant | Use | Notes |
|---|---|---|
| `neutral` | neutralという明示的な意味を持つ文脈。 | 意味を変えずに見た目だけを増やさない。 |
...
- semanticColor: `--success-subtle`, `--success-fg`, `--danger-subtle`, `--danger-fg`, `--fg`, `--fg-muted`, `--surface`, `--border`
- radius: `--radius-full`
...
|---|---|---|---|---|
| `token.success-subtle.value` | `--success-subtle` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.success-fg.value` | `--success-fg` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.danger-subtle.value` | `--danger-subtle` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.danger-fg.value` | `--danger-fg` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.radius-full.value` | `--radius-full` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.surface.value` | `--surface` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Badgeの公開visual contractで用途tokenとして共有する。 |
