...
実行ログのストリーム表示。タイムスタンプ・レベル・展開可能な詳細を持つ。

Machine-readable contract: `design/contracts/components/run-log.contract.json`

## Role

AI & Developer領域でRun Logの責務を1か所にまとめ、類似componentとの選択境界を固定します。PC用とSP用に別componentを作らず、同じ意味とAPIをlayout、viewport、input methodへ適応させます。
...
- 等幅フォント+タブラー数字で桁を揃える。
- ERROR 行は行全体を danger トーンにし、詳細展開を付ける。
- 追尾スクロールはユーザーが上へスクロールしたら停止する。
...
- semanticColor: `--border`, `--border-muted`, `--danger-fg`, `--fg`, `--fg-muted`, `--fg-subtle`, `--focus-ring`, `--surface`, `--surface-muted`, `--table-row-hover`
...
|---|---|---|---|---|
| `token.surface.value` | `--surface` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.border.value` | `--border` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.fg.value` | `--fg` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.surface-muted.value` | `--surface-muted` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.fg-muted.value` | `--fg-muted` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.border-muted.value` | `--border-muted` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
...
| `token.row-h.value` | `--row-h` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.cell-y.value` | `--cell-y` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.focus-ring.value` | `--focus-ring` | `semantic` | - | Run Logの公開visual contractで用途tokenとして共有する。 |
| `token.danger-fg.value` | `--danger-fg` | `semantic` | - | Run LogのHTML showcaseで実際に参照する公開token。 |
| `token.fg-subtle.value` | `--fg-subtle` | `semantic` | - | Run LogのHTML showcaseで実際に参照する公開token。 |
